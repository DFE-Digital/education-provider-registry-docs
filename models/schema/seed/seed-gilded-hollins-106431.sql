...
-- The target model uses seeded nursery_provision_id 2 for No nursery classes.
-- The target model uses seeded sixth_form_provision_id 2 for No sixth form.
-- The source school capacity in the local BAU copy is 22286.
-- The source pupil count in the local BAU copy is 209.
...
JOIN establishment.local_authority AS la ON la.code = 359
WHERE e.urn = 106431
...
SELECT 'obfuscated', NULL, NULL, 'obfuscated', '019', 'WN7 3PQ'
WHERE NOT EXISTS (SELECT 1 FROM establishment.address WHERE postcode = 'WN7 3PQ' AND address_line_1 = 'obfuscated');
INSERT INTO establishment.site (address_id, site_name, uprn)
SELECT a.address_id, NULL, 100012855917
FROM establishment.establishment e
JOIN establishment.address a ON a.postcode = 'WN7 3PQ' AND a.address_line_1 = 'obfuscated'
WHERE e.urn = 106431
  AND NOT EXISTS (SELECT 1 FROM establishment.site s WHERE s.uprn = 100012855917);
...
FROM establishment.establishment e
JOIN establishment.site s ON s.uprn = 100012855917
WHERE e.urn = 106431
...
-- Validation query:
-- SELECT e.urn, la.code AS local_authority_code, e.establishment_number, e.ukprn, e.name,
