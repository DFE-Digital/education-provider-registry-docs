...
#### Do they have a spreadsheet showing a matrix of each establishment subtype and the attributes it has?

There is no spreadsheet matrix, but the equivalent exists in machine-readable form in the SHACL file (`education-provider-shacl.ttl`). The SHACL encodes, for each establishment type subclass, which properties are required, which are permitted, and what cardinality and value constraints apply. It is the authoritative type-to-attribute mapping in the model.

A human-readable matrix has been generated from the SHACL and is available as a CSV download above: [Establishment subtype attribute matrix](./establishment-subtype-attribute-matrix.csv). It covers 58 attributes × 29 establishment types with required / optional / not applicable values.

The taxonomy (`education-provider-taxonomy.ttl`) provides the establishment type hierarchy as SKOS, and the ontology (`education-provider-ontology.ttl`) declares the OWL class hierarchy of subtypes. Both documents provide pointers to the range of subtypes, but neither is the attribute matrix itself.
